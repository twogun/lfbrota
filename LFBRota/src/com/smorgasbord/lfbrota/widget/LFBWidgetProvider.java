...
public class LFBWidgetProvider extends AppWidgetProvider {
    ...
    public void onReceive(Context ctx, Intent intent) {
        ...
        if (action.equals(REFRESH_ACTION)) {
            ...
            sWorkerQueue.post(new Runnable() {
                ...
                public void run() {
                    final ContentResolver r = context.getContentResolver();
                    final Cursor c = r.query(LFBDataProvider.CONTENT_URI, null, null, null, 
                            null);
    ...
    private RemoteViews buildLayout(Context context, int appWidgetId, boolean largeLayout) {
        ...
        if (largeLayout) {
            ...
            // ignored otherwise.
            final Intent onClickIntent = new Intent(context, LFBWidgetProvider.class);
            onClickIntent.setAction(LFBWidgetProvider.CLICK_ACTION);
            onClickIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId);
            onClickIntent.setData(Uri.parse(onClickIntent.toUri(Intent.URI_INTENT_SCHEME)));
            final PendingIntent onClickPendingIntent = PendingIntent.getBroadcast(context, 0,
                    onClickIntent, PendingIntent.FLAG_UPDATE_CURRENT);
            rv.setPendingIntentTemplate(R.id.dayList, onClickPendingIntent);

            // Bind the click intent for the refresh button on the widget
            final Intent refreshIntent = new Intent(context, LFBWidgetProvider.class);
            refreshIntent.setAction(LFBWidgetProvider.REFRESH_ACTION);
            ...
            rv = new RemoteViews(context.getPackageName(), R.layout.widget_daycell);

            // Update the header to reflect the weather for "today"
            Cursor c = context.getContentResolver().query(LFBDataProvider.CONTENT_URI, null,
                    null, null, null);
            if (c.moveToPosition(0)) {
                String header = context.getString(R.string.city_name);
...
class LFBDataProviderObserver extends ContentObserver {
    ...
    public void onChange(boolean selfChange) {
        ...
        mAppWidgetManager.notifyAppWidgetViewDataChanged(
                mAppWidgetManager.getAppWidgetIds(mComponentName), R.id.dayList);
    }
}